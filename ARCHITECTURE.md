...
- Embedding backend switchable via env var — OpenAI API at low volume ($2/mo), local BGE models at high volume (100+ users justifies Render Pro at $85/mo)
...
✅ ANN search via pgvector: cosine distance on
   profile_embedding vs job embedding_vector →
   aspiration blend (0.7 × profile + 0.3 × goals) →
   soft constraints as post-filter → sector
   diversification (60% cap) → top 15
↓
...
|-------|------------------|-----------------|------|-----------------|
| **Profile Agent** | Builds a structured understanding of who the user is and what they want before matching begins. Sets the foundation for all downstream personalization. | 1. Parse resume and extract structured profile (skills, experience, preferences, dealbreakers) 2. Initialize cold start weights for new users 3. Flag user as cold_start: true until 5 feedback signals accumulated 4. Optional enrichment via LinkedIn and GitHub URLs 5. Trigger weight recalibration on preference updates 6. Build intent-weighted embedding text via build_intent_query() — HIGH: skills/title_include (repeated), MEDIUM: seniority/work_modes/goals_text, LOW: preferred_sectors framed as "Target industry:" (not employer names), EXCLUDE: preferred_companies 7. Embed profile on save as background task (update_profile_embedding) 8. Re-embed when role_description changes 9. Intent-conditioned role_description generation: when goals_text or preferred_sectors is set at parse time, the LLM prompt frames role_description around the stated goal — leading with the experience most relevant to the target, presenting other skills as tools brought to that industry (not the candidate's primary identity) 10. Re-generation trigger: when goals_text or preferred_sectors changes on profile save, _regenerate_description_bg rebuilds role_description from original_role_description through the new intent lens and re-embeds automatically — ensures the profile vector tracks what the user wants, not just what they have done | INPUT: Raw resume, preferences, optional LinkedIn/GitHub URL HARD CONSTRAINTS INITIALIZED: Location type, visa requirement, excluded titles, excluded companies SOFT CONSTRAINTS INITIALIZED: Salary floor, role type, experience floor, seniority ceiling COLD START WEIGHTS: skills_match: 0.30, experience_level: 0.15, salary: 0.15, industry_alignment: 0.15, function_type: 0.15, career_trajectory: 0.10 build_intent_query WEIGHTS: goals_text and preferred_sectors were previously excluded — this caused profiles with cross-industry backgrounds (e.g. "AI implementation" for a petroleum engineer seeking oil and gas roles) to generate embeddings pointing at the wrong industry. Both are now included at MEDIUM/LOW weight respectively. INTENT-CONDITIONED GENERATION: role_description is no longer a neutral resume summary when goals are set — the parse prompt instructs the LLM to find resume evidence that supports the stated goal and lead with that. original_role_description is locked at first parse and used as the stable source for re-generation when goals change later. PROCESS: Extract structured profile → set default weights → flag cold start → store OUTPUT: Structured user profile + profile_embedding vector(1536) → PostgreSQL → cold start flag and weights to Orchestration Agent | Python 3.12, FastAPI, SQLAlchemy, Claude Sonnet (profile parsing), OpenAI text-embedding-3-small (profile embedding), PostgreSQL, numpy |
| **Search Agent** | Global job scraper and source health monitor. Fetches all 41 company career pages daily, syncs to DB, embeds new jobs, and tracks per-source reliability. No user context — runs once per day at 3:00 AM UTC. | ✅ 1. Scrape 41 company career pages at 3:00 AM UTC ✅ 2. Deduplicate by URL — ON CONFLICT DO NOTHING ✅ 3. Mark closed jobs inactive (no longer in ATS feed) ✅ 4. Re-activate jobs that reappear in ATS ✅ 5. Enforce 10k job cap — remove oldest inactive jobs with no feedback ✅ 6. Embed new jobs at ingestion via embed_single (OpenAI text-embedding-3-small) ✅ Phase A — 7. HTTP HEAD health check per active job URL — mark inactive if 404/410 ✅ Phase A — 8. Track per-source trust score: jobs_returned, parse_success_rate, dead_link_pct (rolling 30 days) ✅ Phase A — 9. Log warning if source trust score drops below 0.70 ✅ Phase A — 10. Skip scrape for sources with trust score below 0.50
...
| **Filter Agent** | Per-user candidate selector. Applies hard constraints, then uses pgvector ANN search to return the top 15 semantically closest jobs per user for the Matching Agent. | ✅ 1. Apply hard constraints: job type, work mode, location, visa sponsorship, excluded titles, excluded companies ✅ 2. Write pass/fail to job_matches with failure reason ✅ 3. Give benefit of the doubt when job field data is missing ✅ 4. Build intent query (build_intent_query): HIGH weight on skills/title_include, MEDIUM on seniority/work_mode/goals_text, LOW on preferred_sectors ✅ 5. Aspiration blend: 0.7 × profile_embedding + 0.3 × goals_embedding (normalized) — blended only at query time ✅ 6. pgvector ANN search (cosine distance) on hard-passed unseen jobs — LIMIT 50 ✅ 7. Soft constraints as post-filter (salary floor, role type, experience, seniority) ✅ 8. Diversification: cap any single sector at 60% (skip if <20% of results have sector data) AND cap any single company at _COMPANY_CAP=2 slots regardless of sector data. Company cap prevents one employer from filling all 15 slots when their jobs span multiple sector labels or have no sector data. ✅ 9. Return top 15 → Matching Agent NOTE — location filter ordering: _is_us_compatible_location runs BEFORE _contains_us_state and _contains_us_city. This prevents the "IN" (Indiana) state code from matching the "IN" country code in Indian addresses (e.g. "Bangalore, KA, IN, 560066"), which would otherwise cause India-based jobs to pass US-only filters. | INPUT: user profile (profile_embedding, goals_text, hard/soft constraints), job_matches table, jobs.embedding_vector ANN SEARCH: `SELECT jobs ORDER BY embedding_vector <=> query_vector LIMIT 50` on hard-passed unseen jobs FALLBACK: If profile_embedding IS NULL → embed on the fly via embed_single(build_intent_query(profile)) OUTPUT: Top 15 job candidates → Matching Agent. If fewer than 3 → fallback signal to Orchestration Agent | Python 3.12, SQLAlchemy, PostgreSQL, pgvector (HNSW cosine ANN), OpenAI text-embedding-3-small (embed_single + build_intent_query), numpy |
| **Matching Agent** | The AI brain. Scores all filtered jobs in a single LLM call with explicit weighted dimensions. Computes final scores in code. Maximum 2 LLM calls per user per day. | 1. Receive top 10-15 jobs 2. Compress each job to ~300 tokens using spaCy 3. Check dimension_data_available and confidence flags per dimension per job 4. If salary missing — exclude from scoring, re-normalize remaining weights to sum to 1.0 5. Validate all weights sum to 1.0 before every LLM call 6. Batch ALL jobs into single Call 1 7. Compute weighted scores in code — never inside LLM 8. Normalize final scores 9. Flag low confidence jobs (2+ dimensions ambiguous) 10. Conditional Call 2 — structured decision content on top 3, active users only, cached per user_id + job_id + profile_version 11. Invalidate Call 2 cache when profile_version changes, weights shift >0.10 in any dimension, job_updated_at changes, or 7-day TTL expires | INPUT: Top 10-15 compressed jobs, user profile summary, dimension weights, cold start flag, dimension_data_available flags, dimension confidence levels ALLOWED DIMENSIONS — exactly these 6: skills_match, industry_alignment, experience_level, function_type, salary (excluded if data unavailable — weights re-normalized), career_trajectory DIMENSION CONFIDENCE LEVELS (per dimension per job): score: 0.0-1.0 or null, data_available: true/false, confidence: high/medium/low WEIGHT RULES: Sum to 1.0. No dimension above 0.50. No dimension below 0.05. Re-normalize if salary excluded. FUNCTION FLOOR PENALTY: if function_type score < 0.40 the final weighted score is multiplied by 0.70. This prevents fundamental role type mismatches from ranking highly despite strong skills and experience scores. Applied after weighted score calculation, before normalization. SECTOR RULE (injected into system prompt when preferred_sectors is non-empty): job sector outside preferred_sectors → industry_alignment must be ≤ 0.25. Previously the LLM received sector preference as context but had no explicit penalty instruction — it would reason "leadership skills transfer" and score cross-sector jobs moderately rather than low. CALL 1 (always): Batch all jobs + weights + hallucination prevention → parse JSON → compute scores in code → apply function floor penalty → normalize → REORDER PASS (when goals_text or preferred_sectors is set) → rank REORDER PASS (LLM 2): after normalization, a second Haiku call reviews all 15 candidates. Given the candidate's stated goal and background, it returns three outputs: (1) top_3_ids — the 3 jobs that best serve the stated career direction (not just skills overlap); (2) top_3_reasons — one sentence per job written as if addressing the candidate directly about that specific role, no mention of rankings or that 15 jobs were reviewed, stored in call2_content["reorder_reason"] for user-facing display; (3) admin-only outputs: reasoning (one sentence explaining why the order changed) and profile_gap (one sentence on missing profile data limiting quality). If swaps_made=true, normalized score values are redistributed to the new order before delivery selection. Returns meta even when swaps_made=false so top_3_reasons are always stored. Only fires when profile has explicit intent signals (goals_text or preferred_sectors). In the debug pipeline the reorder runs as a separate LLM 2 Score step (skip_reorder=True on LLM 1 Score step); in production both run back-to-back inside MatchAgent.run(). Admin debug LLM 2 results table shows: LLM 1 rank → LLM 2 rank per job, promoted rows (▲ green), demoted rows (▼ red), industry score, plus reasoning and profile_gap banners — all admin-only. CALL 2 (conditional, cached): Produces: why worth pursuing (2-3 sentences), what might hold user back, suggested course gaps, confidence level (high/medium/low), advisor-style summary, not_recommended (bool). not_recommended=true when the role contradicts the candidate's stated direction or the advisor_summary is predominantly negative — even if dimension scores are high. When true: dashboard card shows amber warning icon + "Advisor caution" label instead of match %; drawer replaces "Why this could be a good fit" section with an amber advisory banner. Cache key: user_id + job_id + profile_version. Invalidated by: profile_version change, >0.10 weight shift, job update, 7-day TTL. OUTPUT: Ranked list with per-dimension scores, confidence levels, weighted scores, normalized scores, low confidence flags, Call 2 structured content where cached | Python 3.12, Claude Haiku (Call 1), Claude Sonnet (Call 2 — conditional, cached), spaCy, JSON parsing with regex fallback, FastAPI |
| **Feedback Agent** | The learning engine. Converts every user interaction into structured signals that make tomorrow's matches smarter. Maintains immutable event history and separate current state. | 1. Log all feedback to immutable event log 2. Update job_user_state separately — never overwrite event log 3. Collect signals with standardized values 4. Trigger weight update every 5 signals 5. Immediate trigger on applied, interview, or hired 6. Attribute signals to matching dimensions 7. Extract themes from free-text commentary using Claude Haiku — returns dimension, direction, confidence (high/medium/low → ±0.05/0.03/0.01 delta), and optional hard_exclusion phrase 8. Enforce drift protection — floor 0.05, ceiling 0.50 9. Normalize weights — always sum to 1.0 10. Increment weights_version on every update 11. Persist updated weights 12. Re-embed profile when role_description changes (update_profile_embedding) 13. Outcome-anchored embedding on interview/applied: 0.8 × profile_embedding + 0.2 × job_embedding (normalized) 14. Passive link-clicks (weight=1) log ActivityLog events only — never create Feedback rows | INPUT: Feedback events — type, job_id, dimension scores, timestamp, optional commentary SIGNAL VALUES: thumbs_up: +2, thumbs_down: -2, click: +1, apply_click: +1, applied: +3 (immediate), not_interested: -1, interview: +4 (immediate), hired: +5 (immediate) COMMENTARY API: POST /users/{id}/feedback/event → FeedbackEvent(signal_type="commentary", commentary=text) → background FeedbackAgent.run_commentaries() STATE MANAGEMENT: feedback_event_log — immutable, append only. job_user_state — current status per user-job pair, updated on each interaction. DIMENSION ATTRIBUTION: Strong skills_match on liked jobs → increase skills_match. Disliked for seniority → decrease experience_level. Commentary "wrong function" → adjust function_type. Applied/interview → treat dimension scores as strong anchor. DRIFT PROTECTION: No dimension above 0.50. No dimension below 0.05. Enforced after every update. OUTPUT: Updated normalized weights → increment weights_version → PostgreSQL → available for next cycle | Python 3.12, Claude Haiku (commentary NLP — _COMMENTARY_SYSTEM prompt → structured JSON), SQLAlchemy, PostgreSQL, FastAPI, numpy (embedding blend) |
...
| **Vector Index** | The speed layer. Pre-computes and indexes all job embeddings daily so each user's ANN search returns top candidates in ~2ms regardless of how many jobs exist. | 1. Rebuild index daily at 4:00 AM UTC 2. Embed all active jobs using text-embedding-3-small (1536d) 3. Store in vector index with HNSW algorithm 4. Serve per-user ANN queries in ~2ms 5. Remove inactive job embeddings 6. Scale horizontally as corpus grows | INPUT: All active job descriptions from PostgreSQL EMBEDDING: f"{title}. {description[:500]}" → embed text-embedding-3-small → store HNSW. New jobs embedded at ingestion (search_agent._embed_new_jobs). QUERY: Embed intent query (build_intent_query) → ANN cosine distance → top 50 in ~2ms SCALING: <10K users: pgvector ($0). 10K-50K: Qdrant self-hosted ($50-200/mo). 50K-100K+: Qdrant cluster or Pinecone ($200-500/mo). OUTPUT: Top 50 job IDs → soft filter + sector diversification → top 15 to Matching Agent | OpenAI text-embedding-3-small (current — EMBEDDING_BACKEND=openai, 1536d, $0.02/1M tokens), BAAI/bge-small-en-v1.5 + bge-large-en-v1.5 (future local — EMBEDDING_BACKEND=local, requires Render Pro), pgvector HNSW cosine index → Qdrant at scale, PostgreSQL |
...
| FeedbackEvent | id, feedback_event_id, user_id, job_id, signal_type, signal_value, timestamp, interaction_source, commentary | Immutable event log — append only, never modified. commentary field holds free-text user input submitted via modal or job card. signal_type="commentary" for text feedback events. | feedback_event_id unique per event |
...
|---------|-----|---------|---------|
| All 8 agents | ✅ | - | - |
| pgvector ANN embedding pipeline (text-embedding-3-small, HNSW cosine) | ✅ | - | - |
| Job backfill — 6667 jobs embedded at $0.04 | ✅ | - | - |
| Embed new jobs at ingestion (search_agent) | ✅ | - | - |
| Profile embedding triggers (save + role_description update + outcome-anchor) | ✅ | - | - |
| Aspiration blend at query time (0.7 profile + 0.3 goals) | ✅ | - | - |
| Sector diversification in ANN results (60% cap) | ✅ | - | - |
| Embedding health metrics in TestAgent + /admin/embedding-health endpoint | ✅ | - | - |
| Weekly recap email | ✅ | - | - |
...
| LinkedIn profile import (Sign In with LinkedIn — name, headline, experience pre-fill only; contacts/network access not available via API) | - | ✅ | - |
...
| First-run pipeline trigger on profile completion — POST /pipeline/run-for-user/{id} with 60s timeout + loading overlay | ✅ | - | - |
| Match funnel delivered count fix — filter by delivered_at not pool size | ✅ | - | - |
| Feedback count fix — passive link-clicks excluded from feedback_count metric | ✅ | - | - |
| Apply action uses recordSignal('applied') not thumbs_up Feedback row | ✅ | - | - |
| Text feedback commentary — modal comment box + job card inline input | ✅ | - | - |
| Commentary AI interpretation via Claude Haiku — dimension/direction/confidence/hard_exclusion | ✅ | - | - |
| Per-company cap (_COMPANY_CAP=2) in Filter Agent — prevents single employer dominating all 15 candidate slots | ✅ | - | - |
| build_intent_query includes goals_text (MEDIUM) and preferred_sectors as "Target industry" (LOW) — fixes cross-industry embedding skew | ✅ | - | - |
| Matching Agent SECTOR RULE — LLM scores industry_alignment ≤ 0.25 when job sector outside preferred_sectors | ✅ | - | - |
| Intent-conditioned profile generation — parse prompt frames role_description around stated career goal; _regenerate_description_bg rebuilds and re-embeds when goals_text or preferred_sectors changes | ✅ | - | - |
| Post-scoring reorder pass (LLM 2) — Haiku career advisor reviews all 15 candidates; returns top_3_ids (best career-aligned), top_3_reasons (user-facing per-job fit sentence, no process language, stored in call2_content), reasoning (admin: why order changed), profile_gap (admin: missing data), swaps_made bool; score redistribution preserves display coherence | ✅ | - | - |
| Admin debug page — Check Candidates step removed (redundant); LLM Score split into LLM 1 Score (Haiku batch) + LLM 2 Score (Haiku reorder) as separate triggerable steps; LLM 2 results section shows ranking table (LLM 1 → LLM 2 per job, ▲/▼ indicators), reasoning card, profile gap banner | ✅ | - | - |
| LinkedIn "People in your network" UI removed — LinkedIn Connections API closed to third-party apps since 2015; warm-introduction feature not deliverable; removed from: DetailsDrawer ("People in your network" section), Settings ("Connected sources" section), Privacy page (connected network data bullet) | ✅ | - | - |
| Location filter — India/IN false positive fix: _is_us_compatible_location now runs before _contains_us_state/_contains_us_city so "IN" (country code, India) cannot match Indiana state code in comma-delimited addresses like "Bangalore, KA, IN, 560066" | ✅ | - | - |
| Call 2 not_recommended flag — when advisor_summary is predominantly negative or role contradicts stated career direction, not_recommended=true; dashboard shows amber warning icon + "Advisor caution" instead of match %; drawer shows advisory banner replacing "Why this could be a good fit" section | ✅ | - | - |
...
GET   /admin/embedding-health      (job + profile embedding coverage)
```
